...
def test_full_calibration_and_sweep_reproduce_the_committed_files(tmp_path):
    """Full-size calibration and sweep against the committed results/calibration.json and
    results/sweep.json, value for value, with only the provenance stamp and the latency
    columns excluded -- bitwise on the build that generated them, and otherwise to the
    declared cross-build tolerance (set_lcm.experiments.compare says what was measured and
    why), exactly as tests/test_results_reproduce.py does for the grid."""
    import json
    ...
    for name in ("calibration.json", "sweep.json"):
        ...
        failure = reproduction_failure(name, fresh, committed)
